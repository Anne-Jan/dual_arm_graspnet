...
def control_point_l1_loss_split(pred_control_points,
                          gt_control_points,
                          confidence=None,
                          confidence_weight=None,
                          device="cpu", dual_grasp=False):
    ...
    if confidence is not None:
        ...
        if dual_grasp:
            ...
            ###Old code
            #Split the predicted control points and the ground truth control points
            pred_control_points1 = pred_control_points[0]
            pred_control_points2 = pred_control_points[1]
            gt_control_points_copy = gt_control_points.clone()
            gt_control_points1 = gt_control_points[:,0,:,:]
            gt_control_points2 = gt_control_points_copy[:,1,:,:]
            #Calculate the error
            error1 = torch.sum(torch.abs(pred_control_points1 - gt_control_points1), -1)
            error2 = torch.sum(torch.abs(pred_control_points2 - gt_control_points2), -1)
            ...
            confidence1 = confidence[0]
            confidence2 = confidence[1]
            confidence1 = torch.mean(confidence1, -1)
            confidence2 = torch.mean(confidence2, -1)
            confidence_term1 = torch.mean(
                torch.log(torch.max(
                    confidence1,
                    torch.tensor(1e-10).to(device)))) * confidence_weight
            confidence_term2 = torch.mean(
                torch.log(torch.max(
                    confidence2,
                    torch.tensor(1e-10).to(device)))) * confidence_weight
            return (torch.mean(error1),  torch.mean(error2)), (-confidence_term1, -confidence_term2)
            # print('error', error.shape)
            #Try to make the error bigger, make it inversely proportional to the downscaling factor of the meshes
            # error *= 2.5
            error = torch.mean(error, -1)
...
def min_distance_loss_split(pred_control_points,
                            gt_control_points,
                            confidence=None,
                            confidence_weight=None,
                            threshold=None,
    ...
    return (torch.mean(min_distance_error1), torch.mean(min_distance_error2)), (-confidence_term1, -confidence_term2)
...
def min_distance_loss(pred_control_points,
    ...
    if len(pred_shape) == 4:
        ...
        ###old code that splits the tensor in half
        ...
        if confidence is not None:
            ...
            min_distance_error = torch.mean(min_distance_error)
        ###new code that calculates the error for the dual grasp

        else:
            error = torch.mean(min_distance_error)
            confidence_term = 0.
            min_distance_error = torch.mean(error)
    else:       
...
def kl_divergence_split(mu, log_sigma, device="cpu", dual_grasp=False):
    #split them from bs, 4 to two of bs, 2
    mu1 = mu[0]
    mu2 = mu[1]
    log_sigma1 = log_sigma[0]
    log_sigma2 = log_sigma[1]
    return (torch.mean(
        -.5 * torch.sum(1. + log_sigma1 - mu1**2 - torch.exp(log_sigma1), dim=-1)), torch.mean(
        -.5 * torch.sum(1. + log_sigma2 - mu2**2 - torch.exp(log_sigma2), dim=-1)))
