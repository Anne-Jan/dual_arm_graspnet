...
def grasp_iou(pose1, pose2, bbox1 = [0.03, 0.03, 0.03], bbox2 = [0.03, 0.03, 0.03], threshold = 0.25):
    ...
    # print (iou)
    if iou >= threshold:
        #Check if the rotation is within 45 degrees, if not return 0
        ...
        for jaccard_threshold in all_thresholds:
            ...
            for i, data in enumerate(dataset):
                ...
                for i in range(len(generated_grasps) -1):
                    if i %2 == 0:
                        continue
                    # print('at i', i), 'of', len(generated_grasps)
                ...
                if good_grasps == 0:
                    ...
                    # continue
                # grasps_pairs = []
                # for i in range(len(grasps_to_save) - 1):
                #     for j in range(i+1, len(grasps_to_save)):
                #         grasps_pairs.append(grasps_to_save[i])
                #         grasps_pairs.append(grasps_to_save[j])
