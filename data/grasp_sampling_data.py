...
class GraspSamplingData(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        pos_grasps, pos_qualities, _, _, _, cad_path, cad_scale, all_available_grasps = self.read_grasp_file(
            path)
        meta = {}
        try:
        ...
        all_gt_control_points = utils.transform_control_points_numpy(
            np.array(all_available_grasps), len(all_available_grasps), mode='rt')
        ...
        if len(np.array(output_grasps).shape) == 4:
            ...
            meta['all_grasp_rt'] = np.array(all_available_grasps)
            ...
            # print("all_target cps shape" + str(meta['all_target_cps'].shape))
            # meta['grasp_rt'] = np.array(output_grasps).reshape(
